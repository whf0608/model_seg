...
from MambaCD.changedetection.datasets.make_data_loader1 import MultimodalDamageAssessmentDatset_Inference
...
def map_labels_to_colors(labels, ori_label_value_dict, target_label_value_dict):
    ...
    H, W = labels.shape
    color_mapped_labels = np.zeros((H, W), dtype=np.uint8)
    ...
    for target_label, ori_label in target_to_ori.items():
        ...
        color_mapped_labels[mask] = target_label_value_dict[ori_label]
...
class Trainer(object):
    ...
    def infer(self):
        ...
        # dataset = MultimodalDamageAssessmentDatset(self.args.test_dataset_path, self.args.test_data_name_list,crop_size=self.args.crop_size,  suffix='.tif')
        # dataset = DamageAssessmentDatset(self.args.test_dataset_path, self.args.test_data_name_list, 256, None, 'test')
        dataset =  MultimodalDamageAssessmentDatset_Inference(self.args.test_dataset_path, self.args.test_data_name_list, suffix='.tif')
        ...
        with torch.no_grad():
            for itera, data in enumerate(tqdm(val_data_loader)):
                ...
                output_clfs= np.ones((1024,1024))
                output_locs = np.ones((1024,1024))
                for i in range(2):
                    for j in range(2):
                        sub_pre_change_imgs = pre_change_imgs[:,:,512*(i):512*(i+1),512*(j):512*(j+1)]
                        sub_post_change_imgs = post_change_imgs[:,:,512*(i):512*(i+1),512*(j):512*(j+1)]
                        
                        pre_change_imgs_ = sub_pre_change_imgs.cuda()
                        post_change_imgs_ =  sub_post_change_imgs.cuda()
                        # labels_loc = labels_loc.cuda().long()
                        # labels_clf = labels_clf.cuda().long()
    
                        output_loc, output_clf = self.deep_model(pre_change_imgs_, post_change_imgs_)
                        ...
                        image_name = names[0]+ '_building_damage.png'
                        ...
                        output_clfs[512*(i):512*(i+1),512*(j):512*(j+1)] =  output_clf
                        output_locs[512*(i):512*(i+1),512*(j):512*(j+1)] =  output_loc
                
                cv2.imwrite(os.path.join(self.building_map_T1_saved_path, image_name), output_locs.astype(np.uint8))
                cv2.imwrite(os.path.join(self.change_map_T2_saved_path, image_name), output_clfs.astype(np.uint8))
...
def main():
    ...
    parser.add_argument('--cfg', type=str, default='./configs/vssm1/vssm_base_224.yaml')
    parser.add_argument(
        "--opts",
    ...
    parser.add_argument('--test_dataset_path', type=str, default='/home/wanghaifeng/project_work/datasets/dfc25_track2_trainval/val')
    parser.add_argument('--test_data_list_path', type=str, default='/home/wanghaifeng/whf_work/work_sync/satellite_data/models_config/BRIGHT/dfc25_benchmark/dataset/splitname/val_setlevel.txt')
    ...
    parser.add_argument('--resume', type=str, default='/home/wanghaifeng/whf_work/work_sync/satellite_data/models_config/MambaCD/saved_models/xBD/STMambaBCD_1737470163.7134426/32250_model.pth')
