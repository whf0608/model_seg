...
class Fusion2Backbone(nn.Module):
    def __init__(self, c1, ns=[6, 1, 1, 6, 1, 1], ss=[1, 1, 2, 1, 1, 2], c_size=True):
        ...
        cov_len = [1 + _ for _ in range(len(ss))]

        c = c1
        for i, (n, s, clen) in enumerate(zip(ns, ss, cov_len)):
            ...
            for j in range(clen):
                self.seq3.add_module('ct' + str(i) + '_' + str(j), nn.ConvTranspose2d(c1, c, 3, 3, 0))
                self.seq3.add_module('c2' + str(i) + '_' + str(j), Conv(c1, c, 3, 1, 1))
            c1 = c
